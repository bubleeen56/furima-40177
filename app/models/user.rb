class User < ApplicationRecord
  ...
  devise :database_authenticatable, :registerable,
   ...
   validates :first_name_kanji, presence: true, format: { with: /\A[^\p{Alnum}]*[ぁ-んァ-ン一-龥ー－]+\z/, message: "is invalid. Input full-width characters" }
   validates :first_name, format: { with: /\A[^\p{Alnum}]*\p{Katakana}+[^\p{Alnum}]*\z/, message: "is invalid. Input full-width characters" }
   validates :last_name_kanji, presence: true, format: { with: /\A[^\p{Alnum}]*[ぁ-んァ-ン一-龥ー－]+\z/, message: "is invalid. Input full-width characters" }
